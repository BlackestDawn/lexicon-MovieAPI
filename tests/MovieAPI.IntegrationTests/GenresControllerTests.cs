...
public class GenresControllerTests(IntegrationTestWebAppFactory factory) : IntegrationTestBase(factory)
{
  ...
  {
    var response = await Client.PostAsJsonAsync("/api/genres", TestData.ValidGenre(name: ""));
  ...
  {
    await Client.PostAsJsonAsync("/api/genres", TestData.ValidGenre("Drama", "drama"));
    await Client.PostAsJsonAsync("/api/genres", TestData.ValidGenre("Comedy", "comedy"));
  ...
  public async Task GetGenre_WithExistingId_Returns200()
  {
    ...
    var response = await Client.GetAsync($"/api/genres/{created.Id}");
  ...
  {
    var response = await Client.GetAsync($"/api/genres/{Guid.NewGuid()}");
  ...
  public async Task UpdateGenre_WithExistingId_Returns204AndPersistsChange()
  {
    ...
    var response = await Client.PutAsJsonAsync($"/api/genres/{created.Id}", TestData.ValidGenre("Updated", "updated"));

    Assert.Equal(HttpStatusCode.NoContent, response.StatusCode);

    var fetched = await Client.GetFromJsonAsync<GenreExtendedDto>($"/api/genres/{created.Id}");
  ...
  {
    var response = await Client.PutAsJsonAsync($"/api/genres/{Guid.NewGuid()}", TestData.ValidGenre());
  ...
  public async Task PatchGenre_WithExistingId_Returns204AndPersistsChange()
  {
    ...
    var response = await PatchJsonPatchAsync($"/api/genres/{created.Id}", patch);
  ...
  public async Task DeleteGenre_WithExistingId_Returns204AndSubsequentGetReturns404()
  {
    ...
    var deleteResponse = await Client.DeleteAsync($"/api/genres/{created.Id}");
    Assert.Equal(HttpStatusCode.NoContent, deleteResponse.StatusCode);

    var getResponse = await Client.GetAsync($"/api/genres/{created.Id}");
  ...
  {
    var response = await Client.DeleteAsync($"/api/genres/{Guid.NewGuid()}");
